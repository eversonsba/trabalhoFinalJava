		...
		<div id="container" class="container-fluid">
			<div id="alert" style="${not empty message ? 'display: block;' : 'display: none;'}" class="alert alert-dismissable ${alertType eq 1 ? 'alert-success' : 'alert-danger'}">
	 	 	...
	 	 	<div id="top" class="row">
			    ...
			    <div class="col-md-3">
			        <a href="/ProjetoFinalJava/funcionario/form" class="btn btn-danger pull-right h2"><span class="glyphicon glyphicon-plus" /></span>&nbspAdicionar Funcionario</a>
			    </div>
	     	</div>
			            ...
			            <tbody>
			            	<c:forEach var="funcionarios" items="${funcionarios}">
								<tr>
				                    ...
				                    <td>${funcionarios.getCliente().getNome()}</td>		                    
				                    <td class="actions">
				                        <a class="btn btn-info btn-xs" 
				                           href="${pageContext.request.contextPath}/funcionario/update?funcionarioId=${funcionarios.getId()}" >
				                           <span class="glyphicon glyphicon-edit"></span>
				                        </a>
				                    </td>
				                    <td class="actions">
				                        <a class="btn btn-danger btn-xs modal-remove" funcionario-id="${funcionarios.getId()}" 
				                           funcionario-nome="${funcionarios.getNome()}" data-toggle="modal" 
		...
		<script type="text/javascript">
			$(document).ready(function() {
			    ...
			    $(".modal-remove").click(function () {
		            ...
		            $(".modal-body #hiddenValue").text("Funcionario '"+funcionarioNome+"'");
