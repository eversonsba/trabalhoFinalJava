		...
		<div id="container" class="container-fluid">
			<div id="alert" style="${not empty message ? 'display: block;' : 'display: none;'}" class="alert alert-dismissable ${alertType eq 1 ? 'alert-success' : 'alert-danger'}">
	 	 	...
	 	 	<div id="top" class="row">
			    ...
			    <div class="col-md-3">
			        <a href="/ProjetoFinalJava/cliente/form" class="btn btn-danger pull-right h2"><span class="glyphicon glyphicon-plus" /></span>&nbspAdicionar Clientes</a>
			    </div>
	     	</div>
			            ...
			            <tbody>
			            	<c:forEach var="clientes" items="${clientes}">
								<tr>
				                    ...
				                    <td>${clientes.getCnpj()}</td>				                    
				                    <td class="actions">
				                        <a class="btn btn-info btn-xs" 
				                           href="${pageContext.request.contextPath}/cliente/update?id=${clientes.getCnpj()}" >
				                           <span class="glyphicon glyphicon-edit"></span>
				                        </a>
				                    </td>
				                    <td class="actions">
				                        <a class="btn btn-danger btn-xs modal-remove"
				                           cliente-cnpj="${clientes.getCnpj()}" 
				                           cliente-nome="${clientes.getNome()}" data-toggle="modal" 
		...
		<script type="text/javascript">
			$(document).ready(function() {
			    ...
			    $(".modal-remove").click(function () {
			    	 var nome = $(this).attr('cliente-nome');
			            var cnpj = $(this).attr('cliente-cnpj');
			            $(".modal-body #hiddenValue").text("Cliente '"+nome+"'");
			            $("#id").attr( "value", cnpj);
			            $("#form").attr( "action","cliente/delete");
		        })
			});
		</script>
	</body>
